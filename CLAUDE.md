...
- `generate_digest.py` — `_ensure_complete_descriptions(md)` post-processes LLM output before HTML conversion; `_load_seen_urls()` / `_save_seen_urls()` manage the freshness filter; `_prefer_unseen()` sorts item pools so unseen items come first; `markdown_to_html()` bullet regex accepts titles with or without `**` bold markers for LLM compatibility; `feature_excluded` set blocks Docs Changelog, HN, and Reddit from Features; `news_sources` set includes Docs Changelog, HN, and Reddit for General News; General News pool is sorted by two-tier key: (is_fresh_last_12h, score) — official sources get a baseline score of 50; `_prefer_unseen()` is applied after score sort so unseen items still surface first within each tier
