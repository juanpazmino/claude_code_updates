...
Before any design or copy work, read `/Users/coding/Desktop/AI_consultant/BRAND.md`.
...
- **LLM:** Anthropic Sonnet 5 (`claude-sonnet-5`) for the digest summarizer; Haiku 4.5 (`claude-haiku-4-5-20251001`) for the chat assistant — requires `ANTHROPIC_API_KEY` in `.env` (local) and as a Vercel env var (chat function)
...
- `api/chat.js` — Vercel Function (Node ESM), sole holder of `ANTHROPIC_API_KEY` on the server side; streams Haiku 4.5 with the full `knowledge.json` inlined in the system prompt (`cache_control: ephemeral`); replies in the user's language; in-memory rate limit 20 req/10 min per IP; request validation (max 24 messages × 1000 chars)
